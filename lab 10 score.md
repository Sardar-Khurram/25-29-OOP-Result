# Lab 10 Evaluation Master Summary (File Handling in C++)

This master summary compiles the evaluation results for **Lab 10 (File Handling in C++)** across all 42 students. Each student's submission has been manually inspected line-by-line, compiled using `g++ -std=c++17 -Wall -Wextra`, checked for exact raw and structural plagiarism, and graded against the official rubric.

## 📊 **Class Grade Summary Table**

| Roll Number | Student Name | Folder Location | C1 (3) | C2 (3) | C3 (3) | C4 (3) | Core (12) | Plag Penalty | Final Marks | Status |
| :--- | :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| 2024-SE-05 | Qurat-Ul-ain Fatima | `file handling\` | 3 | 3 | 2 | 3 | 11 | -3 | **8 / 12** | Passed (Good) |
| 2024-SE-08 | Syed Ahmed Gillani | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |
| 2024-SE-12 | Adeela Bashir | `file handling\` | 3 | 3 | 2 | 3 | 11 | -3 | **8 / 12** | Passed (Good) |
| 2024-SE-18 | Syed Abdul Rehman Naseem | `LAB#10\` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2024-SE-23 | Muqaddas Kiani | `WEEK10\` | 3 | 3 | 3 | 2 | 11 | 0 | **11 / 12** | Passed (Excellent) |
| 2024-SE-27 | Muhammad Arslan | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |
| 2024-SE-28 | Raja Wajihullah Khan | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |
| 2024-SE-29 | Sakeena Bano | `file handling\` | 3 | 3 | 2 | 3 | 11 | -3 | **8 / 12** | Passed (Good) |
| 2024-SE-30 | Maryam shaheen | `file handling\` | 3 | 3 | 2 | 3 | 11 | -3 | **8 / 12** | Passed (Good) |
| 2024-SE-32 | Muhammad Hasham | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |
| 2024-SE-33 | Momna Imran | `LAB 10 oop.cpp` | 3 | 3 | 3 | 2 | 11 | 0 | **11 / 12** | Passed (Excellent) |
| 2024-SE-36 | Sahadat Ali | `LAB_11_File_Handling_Sahadat_ali\ (Swapped folder)` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-01 | Syeda Mariam Gillani | `Lab-10\` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-02 | Shahnza Munir | `Lab 9 (File Handling)\ (Swapped folder)` | 3 | 3 | 3 | 3 | 12 | -3 | **9 / 12** | Passed (Good) |
| 2025-SE-03 | Muhammad Zain Mughal | `Lab 10\` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-04 | Wajeeha Batool | `Lab 10 (File Handling)\` | 3 | 3 | 3 | 3 | 12 | -3 | **9 / 12** | Passed (Good) |
| 2025-SE-05 | Ali Zain-Ul-Abdin | `LAB 10\` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-06 | Maryam Jamil | `Lab 10\` | 3 | 3 | 3 | 3 | 12 | -3 | **9 / 12** | Passed (Good) |
| 2025-SE-10 | Misbah Khurshid | `Lab 10\` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-11 | Abdul Hadi Kiani | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |
| 2025-SE-12 | Adeeba Mansoor Qureshi | `Lab Tasks\10_Lab(File Handling)\` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-13 | Areeba Taqdees Mir | `WEEK 10\` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-14 | Komal Shafique | `Lab 10\` | 3 | 3 | 3 | 3 | 12 | -3 | **9 / 12** | Passed (Good) |
| 2025-SE-15 | Samia Arshad | `Week 10\` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-16 | Ihtisham Hussain Raja | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |
| 2025-SE-17 | Muqaddas Majeed | `OOP lab 10\` | 3 | 3 | 3 | 3 | 12 | -3 | **9 / 12** | Passed (Good) |
| 2025-SE-18 | Tahreem Rasheed | `File handling\` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-21 | Noor Fatima Kiani | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |
| 2025-SE-22 | Noor Ul Ain Abbasi | `Lab 10(File handling)\` | 3 | 3 | 3 | 3 | 12 | -3 | **9 / 12** | Passed (Good) |
| 2025-SE-28 | Syed Fasih UL Hassan Gardezi | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |
| 2025-SE-29 | Shiza Mukhtar | `Lab 9 tasks\ (Swapped folder)` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-30 | Malik Waqar Latif | `Lab 9\ (Swapped folder)` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-31 | Khawaja Absar Ali | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |
| 2025-SE-32 | Laiba Imtiaz | `Lab9-3\ (Swapped folder)` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-33 | Rohail Abbas | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |
| 2025-SE-34 | Niqash Ali | `OOP\Week 10\` | 3 | 3 | 3 | 3 | 12 | -3 | **9 / 12** | Passed (Good) |
| 2025-SE-35 | Muhammad Zakarya | `Week 10\` | 3 | 3 | 3 | 3 | 12 | -3 | **9 / 12** | Passed (Good) |
| 2025-SE-36 | Tanzeela Rehman | `lab 9\ (Swapped folder)` | 3 | 3 | 3 | 3 | 12 | -3 | **9 / 12** | Passed (Good) |
| 2025-SE-38 | Ammar Yasir | `Lab No 10\` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-40 | Danish Iqbal | `LAB 10 File Handling\` | 3 | 3 | 3 | 3 | 12 | 0 | **12 / 12** | Passed (Excellent) |
| 2025-SE-41 | Abdul Jalal Kazmi | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |
| 2025-SE-42 | Muhammad Arif | `N/A` | 0 | 0 | 0 | 0 | 0 | 0 | **0 / 12** | Absent / Empty Task Folder |

---
### 🔑 **Rubric Criteria Key:**
* **C1 (File Creation & Writing):** Files are created correctly with the required names (`notes.txt`, `students.txt`), and all required text/student data is written accurately without overwriting existing content.
* **C2 (File Reading & Content Display):** Files are opened correctly in read mode (`ifstream`), complete contents are read, and required information is displayed accurately on the screen.
* **C3 (Line Counting & File Appending/Copying):** Required line counting is accurate, data is appended without overwriting existing content (`ios::app`), and file-copying functionality is correctly implemented where required.
* **C4 (File Handling, Execution & Output):** Files are opened/closed appropriately, programs compile and execute successfully, and required output is clear and correct for all tasks.

## 📝 **Detailed Rubric & Deduction Explanations (Student-by-Student)**

* **2024-SE-05 - Qurat-Ul-ain Fatima** (Final: **8/12**):
  - *Folder Location:* `file handling\`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 All 4 source code files are 100% exact raw byte-for-byte copy-matches with students `2024-SE-12`, `2024-SE-29`, and `2024-SE-30`.; Line Counting & File Appending/Copying (2/3): Good. Deducted 1 mark because Task 3 (Copy Content from One File to Another) was replaced with a duplicate line-counting program instead of file copying.

* **2024-SE-08 - Syed Ahmed Gillani** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

* **2024-SE-12 - Adeela Bashir** (Final: **8/12**):
  - *Folder Location:* `file handling\`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 All 4 source code files are 100% exact raw byte-for-byte copy-matches with students `2024-SE-05`, `2024-SE-29`, and `2024-SE-30`.; Line Counting & File Appending/Copying (2/3): Good. Deducted 1 mark because Task 3 (Copy Content from One File to Another) was replaced with a duplicate line-counting program instead of file copying.

* **2024-SE-23 - Muqaddas Kiani** (Final: **11/12**):
  - *Folder Location:* `WEEK10\`
  - *Audit Details:* File Handling, Execution & Output (2/3): Good. Deducted 1 mark under execution because Tasks 1-3 were commented out in the single merged week10.cpp file.

* **2024-SE-27 - Muhammad Arslan** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

* **2024-SE-28 - Raja Wajihullah Khan** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

* **2024-SE-29 - Sakeena Bano** (Final: **8/12**):
  - *Folder Location:* `file handling\`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 All 4 source code files are 100% exact raw byte-for-byte copy-matches with students `2024-SE-05`, `2024-SE-12`, and `2024-SE-30`.; Line Counting & File Appending/Copying (2/3): Good. Deducted 1 mark because Task 3 (Copy Content from One File to Another) was replaced with a duplicate line-counting program instead of file copying.

* **2024-SE-30 - Maryam shaheen** (Final: **8/12**):
  - *Folder Location:* `file handling\`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 All 4 source code files are 100% exact raw byte-for-byte copy-matches with students `2024-SE-05`, `2024-SE-12`, and `2024-SE-29`.; Line Counting & File Appending/Copying (2/3): Good. Deducted 1 mark because Task 3 (Copy Content from One File to Another) was replaced with a duplicate line-counting program instead of file copying.

* **2024-SE-32 - Muhammad Hasham** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

* **2024-SE-33 - Momna Imran** (Final: **11/12**):
  - *Folder Location:* `LAB 10 oop.cpp`
  - *Audit Details:* File Handling, Execution & Output (2/3): Good. Deducted 1 mark under execution because Tasks 1-3 were commented out in the single merged LAB 10 oop.cpp file.

* **2024-SE-36 - Sahadat Ali** (Final: **12/12**):
  - *Folder Location:* `LAB_11_File_Handling_Sahadat_ali\ (Swapped folder)`
  - *Audit Details:* Full Marks. Flawless implementation.

* **2025-SE-02 - Shahnza Munir** (Final: **9/12**):
  - *Folder Location:* `Lab 9 (File Handling)\ (Swapped folder)`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 Source code files are 100% matched with student `2025-SE-36` (Muhammad Zakria) who submitted identical source code files including identical comments and credentials.

* **2025-SE-04 - Wajeeha Batool** (Final: **9/12**):
  - *Folder Location:* `Lab 10 (File Handling)\`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 All 4 source code files are 100% exact raw byte-for-byte copy-matches (including comments and variable naming) with student `2025-SE-22` (Noor Ul Ain Abbasi).

* **2025-SE-06 - Maryam Jamil** (Final: **9/12**):
  - *Folder Location:* `Lab 10\`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 All 4 source code files are 100% exact raw byte-for-byte copy-matches with student `2025-SE-14` (Komal Shafique).

* **2025-SE-11 - Abdul Hadi Kiani** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

* **2025-SE-14 - Komal Shafique** (Final: **9/12**):
  - *Folder Location:* `Lab 10\`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 All 4 source code files are 100% exact raw byte-for-byte copy-matches with student `2025-SE-06` (Maryam Jamil), including Maryam Jamil's name and roll number inside the append file logic.

* **2025-SE-16 - Ihtisham Hussain Raja** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

* **2025-SE-17 - Muqaddas Majeed** (Final: **9/12**):
  - *Folder Location:* `OOP lab 10\`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 Source code files for Task 2 and Task 3 are 100% exact raw byte-for-byte copy-matches with students `2025-SE-04` and `2025-SE-22`.

* **2025-SE-21 - Noor Fatima Kiani** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

* **2025-SE-22 - Noor Ul Ain Abbasi** (Final: **9/12**):
  - *Folder Location:* `Lab 10(File handling)\`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 All 4 source code files are 100% exact raw byte-for-byte copy-matches (including comments and variable naming) with student `2025-SE-04` (Wajeeha Batool).

* **2025-SE-28 - Syed Fasih UL Hassan Gardezi** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

* **2025-SE-29 - Shiza Mukhtar** (Final: **12/12**):
  - *Folder Location:* `Lab 9 tasks\ (Swapped folder)`
  - *Audit Details:* Full Marks. Flawless implementation.

* **2025-SE-30 - Malik Waqar Latif** (Final: **12/12**):
  - *Folder Location:* `Lab 9\ (Swapped folder)`
  - *Audit Details:* Full Marks. Flawless implementation.

* **2025-SE-31 - Khawaja Absar Ali** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

* **2025-SE-32 - Laiba Imtiaz** (Final: **12/12**):
  - *Folder Location:* `Lab9-3\ (Swapped folder)`
  - *Audit Details:* Full Marks. Flawless implementation.

* **2025-SE-33 - Rohail Abbas** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

* **2025-SE-34 - Niqash Ali** (Final: **9/12**):
  - *Folder Location:* `OOP\Week 10\`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 Source code files for Task 2 and Task 3 are 100% exact raw byte-for-byte copy-matches with student `2025-SE-35` (Muhammad Zakarya).

* **2025-SE-35 - Muhammad Zakarya** (Final: **9/12**):
  - *Folder Location:* `Week 10\`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 Source code files for Task 2 and Task 3 are 100% exact raw byte-for-byte copy-matches with student `2025-SE-34` (Zuhaib Ahmad).

* **2025-SE-36 - Tanzeela Rehman** (Final: **9/12**):
  - *Folder Location:* `lab 9\ (Swapped folder)`
  - *Audit Details:* Plagiarism Penalty: **Plagiarism Penalty of **-3 marks** applied.**
 All 4 source code files are 100% exact raw copy-matches with student `2025-SE-02` (Shahnza Munir), including Shahnza's name and roll number inside the append file logic.

* **2025-SE-41 - Abdul Jalal Kazmi** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

* **2025-SE-42 - Muhammad Arif** (Final: **0/12**):
  - *Folder Location:* `N/A`
  - *Audit Details:* File Creation & Writing (0/3): Absent.; File Reading & Content Display (0/3): Absent.; Line Counting & File Appending/Copying (0/3): Absent.; File Handling, Execution & Output (0/3): Absent.

## ⚠️ **Plagiarism Audit Detail**

A flat **-3 marks plagiarism penalty** has been applied to students sharing 100% matched code structures:

1. **Qurat-Ul-Qurat-Ul-ain Fatima (2024-SE-05)**, **Adeela Bashir (2024-SE-12)**, **Sakeena Bano (2024-SE-29)**, & **Maryam shaheen (2024-SE-30)**
   - 100% exact raw byte-for-byte copy match across all 4 files (including duplicate line count in Task 3).

2. **Wajeeha Batool (2025-SE-04)**, **Muqaddas Majeed (2025-SE-17)**, & **Noor Ul Ain Abbasi (2025-SE-22)**
   - 100% exact raw byte-for-byte copy match across tasks.

3. **Maryam Jamil (2025-SE-06)** & **Komal Shafique (2025-SE-14)**
   - 100% exact raw byte-for-byte copy match across all 4 files (including Maryam Jamil's credentials in Komal's append file).

4. **Niqash Ali (2025-SE-34)** & **Muhammad Zakarya (2025-SE-35)**
   - 100% exact raw byte-for-byte copy match across Task 2 and Task 3.

5. **Shahnza Munir (2025-SE-02)** & **Tanzeela Rehman (2025-SE-36)**
   - 100% exact copy across all 4 files (including Shahnza's credentials in Zakria's append file).
